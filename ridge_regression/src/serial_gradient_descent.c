...
void serial_gradient_descent(matrix* X, matrix* y, matrix* theta, const unsigned int num_iter, const float learning_rate, const float lambda){
    ...
    for(unsigned int i = 0; i < num_iter; i++){
        serial_comp_step_one(X, theta, y, placeholder);
        serial_comp_step_two(X_transpose, placeholder, gradient);
        serial_update_weights(theta, gradient, learning_rate, lambda);
    }
